...
void solve(LinkedList h1, LinkedList h2){
    ...
    LinkedList p=h1->next, pre=NULL, nex=p->next;
    while(p!=NULL){
        nex=p->next;
        p->next=pre;
        pre=p;
...
int main(){
    Node *head1=(Node*)malloc(sizeof(Node)), *head2=(Node*)malloc(sizeof(Node)), *head=(Node*)malloc(sizeof(Node));
    head1->next=NULL, head2->next=NULL, head->next=NULL;
