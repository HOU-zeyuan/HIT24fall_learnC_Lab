...
void calendar_frame(int m, int y)
{ // 生成月历前三行
    if (m >= 1 && m <= 12)
        strncpy(calendar[0] + 10, months[m - 1], 9); // 将月份复制到月历第一行
    else
    ...
    for (int i = 0; i < 38; i++)
        calendar[1][i] = '-';
    for (int i = 2, j = 0; i < 38, j < 7; i += 5, j++)
        strncpy(calendar[2] + i, week[j], 3); // 将星期几复制到月历第二行
}
...
int main()
{
    ...
    for (int i = 0; i < judge_month(month, year); i++)
    { // 输入日期
